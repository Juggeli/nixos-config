{
  flake.nixosModules.haruka-log-analyzer =
    { config, pkgs, ... }:
    let
      hoursBack = 24;
      maxLines = 4000;
      ...
      outputDir = "/var/log/log-analyzer";

      analyzerScript = pkgs.writeShellScript "log-analyzer" ''
            set -euo pipefail

            API_KEY=$(cat "${config.age.secrets.openrouter-api.path}")
            OUTPUT_DIR="${outputDir}"
            REPORT_FILE="$OUTPUT_DIR/$(date +%Y-%m-%d).md"
            HOURS_BACK="${toString hoursBack}"
            MAX_LINES="${toString maxLines}"
            MAX_DUPES="${toString maxDuplicates}"
            MODEL="${model}"
        ...
        **Log lines analyzed:** $TOTAL_LINES
        ...
        EOF
            ...
            NTFY_TOPIC=$(cat "${config.age.secrets.ntfy-topic.path}")
