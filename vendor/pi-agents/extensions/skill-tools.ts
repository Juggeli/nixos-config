    ...
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (metadata.isSymbolicLink()) {
        ...
        addDiagnostic(
          ...
          "error",
          "unsupported_file",
          `${label} contains an unsupported file type: ${relativePath}`,
        );
      }
    }
  };
...
function treeHash(files: SkillFile[]): string {
  return sha256(
    files
      .map((file) => `${file.relativePath}\0${file.mode}\0${file.sha256}`)
      .join("\n"),
  );
}
...
function textDiff(
  ...
  const oldMetadata =
    oldPath && fs.existsSync(oldPath) ? fs.statSync(oldPath) : undefined;
  if (newMetadata.size > 100_000 || (oldMetadata?.size ?? 0) > 100_000)
    return undefined;
  return unifiedDiff(
    relativePath,
    oldPath && fs.existsSync(oldPath) ? fs.readFileSync(oldPath, "utf8") : "",
...
function renderDiff(changes: SkillChange[]): string {
  ...
  for (const change of changes) {
    const body =
      change.diff ||
      `${change.status.toUpperCase()} ${change.path} (${change.size} bytes; binary or large text)`;
    if (sections.join("\n\n").length + body.length > 20_000) {
      sections.push(
        `[diff truncated; ${changes.length - sections.length} additional file(s)]`,
      );
      break;
    }
    sections.push(body);
...
function formatValidation(result: SkillValidationResult): string {
  const lines = [
    ...
    result.existing
      ? "Target already exists; files absent from the draft will be preserved."
      : "Target is new.",
  ];
  ...
  lines.push(
    `Files: ${result.files.length}; changes: ${result.changes.length}`,
  );
...
export function registerSkillTools(pi: ExtensionAPI): void {
  ...
  pi.on("session_start", () => {
    try {
      pi.setActiveTools(
        pi.getActiveTools().filter((name) => !toolNames.has(name)),
      );
    } catch {
  ...
  function validate(
    ...
    const { draftPath, targetPath } = skillPaths(
      ctx.cwd,
      params.scope,
      params.name,
    );
    if (!fs.existsSync(draftPath))
      addDiagnostic(
        ...
        "error",
        "draft_missing",
        `Draft does not exist: ${draftPath}`,
      );
    else if (
      fs.lstatSync(draftPath).isSymbolicLink() ||
      !fs.lstatSync(draftPath).isDirectory()
    ) {
      addDiagnostic(
        diagnostics,
        "error",
        "invalid_draft",
        "Skill draft must be a regular directory, not a symlink",
      );
    }
    ...
    if (
      !diagnostics.some(
        (item) =>
          item.code === "draft_missing" || item.code === "invalid_draft",
      )
    ) {
      try {
        draftFiles = walkFiles(draftPath, diagnostics, "Draft");
      } catch (error) {
        addDiagnostic(
          diagnostics,
          "error",
    ...
    const skillFile = draftFiles.find(
    ...
    if (!skillFile) {
      addDiagnostic(
        ...
        "error",
        "skill_missing",
        "Draft must contain SKILL.md at its root",
      );
    } else {
      try {
        ...
        for (const field of Object.keys(frontmatter)) {
          if (!FRONTMATTER_FIELDS.has(field))
            addDiagnostic(
              ...
              "warning",
              "unknown_field",
              `Unknown frontmatter field ignored by pi: ${field}`,
            );
        }
        if (!body.trim())
          addDiagnostic(
        ...
        if (body.split("\n").length > 500)
          addDiagnostic(
            ...
            "warning",
            "long_skill",
            "SKILL.md exceeds the recommended 500 lines; move details to references/",
          );
      ...
      try {
        if (
          ...
          addDiagnostic(
            ...
            "error",
            "invalid_target",
            "Skill target must be a regular directory, not a symlink",
          );
    ...
    if (fs.existsSync(opposite))
      addDiagnostic(
        ...
        "warning",
        "scope_collision",
        `A skill with this name also exists at ${opposite}`,
      );
    ...
    const changes = draftFiles
      .filter(
        (file) =>
          targetByPath.get(file.relativePath)?.sha256 !== file.sha256 ||
          targetByPath.get(file.relativePath)?.mode !== file.mode,
      )
    ...
    return {
      ...
      targetSha256:
        existing && targetFiles.length
          ? treeHash(targetFiles)
          : existing
            ? sha256("")
            : undefined,
      normalized,
  ...
  pi.registerTool({
    ...
    description:
      "Validates a staged skill directory and previews additions or modifications without writing the installed skill.",
    promptSnippet: "Validate a staged skill draft without saving it",
    promptGuidelines: [
      "Use only for a requested dry run or after correcting a rejected draft; skill_save validates automatically.",
    ],
    parameters: CandidateSchema,
    async execute(_id, params, _signal, _update, ctx) {
      if (!SKILL_NAME_PATTERN.test(params.name) || params.name.length > 64)
        return runValidation(ctx, params);
      const paths = skillPaths(ctx.cwd, params.scope, params.name);
      return withFileMutationQueue(paths.draftPath, () =>
        withFileMutationQueue(paths.targetPath, () =>
          runValidation(ctx, params),
        ),
      );
    },
  });
  ...
  pi.registerTool({
    ...
    label: "Save skill",
    description:
      "Validates and atomically overlays a staged skill directory onto its authorized global or project target after confirmation.",
    promptSnippet:
      "Validate and save a staged skill to its exact authorized directory",
    promptGuidelines: [
    ...
    async execute(_id, params, signal, _update, ctx) {
      ...
      const paths = skillPaths(ctx.cwd, params.scope, params.name);
      return withFileMutationQueue(paths.draftPath, () =>
        withFileMutationQueue(paths.targetPath, async () => {
          const result = validate(ctx, params);
          if (!result.valid)
            return {
              content: [
          ...
          if (!result.changes.length)
            return {
              content: [
                {
                  type: "text" as const,
                  text: `No changes: ${result.targetPath}`,
                },
              ],
              details: result,
            };
          if (!ctx.hasUI)
            return {
              content: [
                {
                  type: "text" as const,
                  text: "Skill not saved: every write requires interactive confirmation.",
                },
              ],
              details: result,
              isError: true,
          ...
          if (!approved)
            return {
              content: [
                {
                  type: "text" as const,
                  text: "Skill not saved: write declined by user.",
                },
              ],
              details: result,
              isError: true,
          ...
          if (
            !current.valid ||
            current.draftSha256 !== result.draftSha256 ||
            current.targetSha256 !== result.targetSha256
          ) {
            return {
              content: [
                {
                  type: "text" as const,
                  text: "Skill not saved: draft or target changed after confirmation; save again.",
                },
              ],
              details: current,
              isError: true,
          ...
          const temporary = fs.mkdtempSync(
            path.join(parent, `.${params.name}.new-`),
          );
          const previous = path.join(
            parent,
            `.${params.name}.old-${crypto.randomUUID()}`,
          );
          let targetMoved = false;
          try {
            if (result.existing) {
              for (const entry of fs.readdirSync(result.targetPath))
                fs.cpSync(
                  path.join(result.targetPath, entry),
            ...
            const expected = treeHash(
              walkFiles(temporary, [], "Staged target"),
            );
            if (result.existing) {
            ...
            if (
              treeHash(walkFiles(result.targetPath, [], "Saved target")) !==
              expected
            )
