...
export interface SessionStats {
  ...
  toolCalls: {
    ...
    byTool: Record<
      string,
      { total: number; succeeded: number; failed: number; unresolved: number }
    >;
  };
  usage: {
    input: number;
...
async function resolveJsonlPath(
  ...
  if (!metadata.isFile())
    throw new Error(`Session path is not a regular file: ${resolved}`);
  if (path.extname(resolved).toLowerCase() !== ".jsonl")
    throw new Error(`Session path must end in .jsonl: ${resolved}`);
...
async function* readLines(
  ...
  const abort = () =>
    stream.destroy(new DOMException("Session read aborted", "AbortError"));
...
function markerFromEntry(entry: JsonObject): AgentMarker | undefined {
  ...
  if (
    entry.customType === SYSTEM_PROMPT_TYPE &&
    typeof entry.data.agentName === "string"
  ) {
...
function buildBranch(
  entries: Map<string, IndexedEntry>,
  leafId: string | null,
): { branch: IndexedEntry[]; issue?: string } {
  ...
  while (id) {
    if (seen.has(id))
      return {
        ...
        issue: `Cycle detected at entry ${id}`,
      };
    seen.add(id);
...
async function indexSession(
  ...
  for await (const item of readLines(resolved.path, signal)) {
    ...
    leafId = entry.id;
    if (entry.timestamp && Number.isFinite(Date.parse(entry.timestamp)))
  ...
  return {
    ...
    changedDuringRead:
      after.size !== resolved.size || after.mtimeMs !== resolved.mtimeMs,
  };
}
...
function agentSegments(
  branch: IndexedEntry[],
  agentName: string,
): { segments: AgentSegment[]; activeAtLeaf: boolean } {
  ...
  for (let i = 0; i < branch.length; i++) {
    ...
    if (!marker) continue;
    const establishesAgent =
      marker.kind === "active" ||
      (marker.kind === "system-prompt" && currentAgent === undefined);
...
async function* jsonlFiles(
  ...
  if (!metadata.isDirectory())
    throw new Error(`Session root is not a directory: ${root}`);
...
export async function findAgentSessions(
  ...
  const excludedPath = options.excludePath
    ? (await resolveJsonlPath(options.excludePath, options.signal)).path
    : undefined;
  const matches: Array<{
    path: string;
    sessionId: string;
  ...
  for await (const filePath of jsonlFiles(
    options.sessionRoot ?? path.join(homedir(), ".pi", "agent", "sessions"),
    options.signal,
  )) {
    try {
  ...
  matches.sort((a, b) => {
    const byTimestamp =
      (b.lastTimestamp ? Date.parse(b.lastTimestamp) : -Infinity) -
      (a.lastTimestamp ? Date.parse(a.lastTimestamp) : -Infinity);
...
function selection(
  ...
  if (scope === "active-branch")
    return new Set(index.branch.map((entry) => entry.id));
  if (!agentName)
    throw new Error("agentName is required when scope is agent-segment");
  const segment = agentSegments(index.branch, agentName).segments.at(-1);
  if (!segment)
    throw new Error(`Agent ${agentName} was not found on the active branch`);
  return new Set(
    index.branch
      .filter(
        (entry) =>
          entry.line >= segment.startLine && entry.line <= segment.endLine,
      )
      .map((entry) => entry.id),
  );
}
...
export async function calculateSessionStats(
  ...
  if (options.agentName && !AGENT_NAME_PATTERN.test(options.agentName))
    throw new Error(`Invalid agent name: ${options.agentName}`);
  const index = await indexSession(inputPath, options.signal);
  const selected = selection(index, scope, options.agentName);
  const calls = new Map<
    string,
    { name: string; entry: SessionLandmark; status?: "succeeded" | "failed" }
  >();
  ...
  const remember = (items: SessionLandmark[], item: SessionLandmark) => {
  ...
  await forEachSelectedEntry(
    index,
    selected,
    options.signal,
    (entry, line, rawLine) => {
      selectedEntries++;
      landmarks.largestEntries.push(
        landmark(entry, line, { chars: rawLine.length }),
      );
      landmarks.largestEntries.sort((a, b) => (b.chars ?? 0) - (a.chars ?? 0));
      landmarks.largestEntries.length = Math.min(
        10,
        landmarks.largestEntries.length,
      );
      if (entry.type === "compaction") {
        compactions++;
        remember(landmarks.compactions, landmark(entry, line));
      }
      if (entry.type === "custom" && markerFromEntry(entry))
        remember(landmarks.agentTransitions, landmark(entry, line));
      if (entry.type !== "message" || !isObject(entry.message)) return;
      const message = entry.message;
      if (message.role === "user")
        remember(landmarks.userRequests, landmark(entry, line));
      if (message.role === "assistant") {
        if (message.stopReason === "error") assistantErrors++;
        if (message.stopReason === "stop")
          remember(landmarks.finalAnswers, landmark(entry, line));
        if (isObject(message.usage)) {
          ...
          if (isObject(message.usage.cost)) {
            ...
            usage.cost.cacheWrite += finiteNumber(
        ...
        byModel[model] =
          (byModel[model] ?? 0) +
          (isObject(message.usage) && isObject(message.usage.cost)
            ? finiteNumber(message.usage.cost.total)
            : 0);
        ...
        for (const block of message.content) {
          if (
            ...
            continue;
          const callEntry = landmark(entry, line, {
            toolName: block.name,
            toolCallId: block.id,
          });
          const call = { name: block.name, entry: callEntry } as {
            name: string;
            entry: SessionLandmark;
            status?: "succeeded" | "failed";
          };
          const early = earlyResults.get(block.id);
          if (early !== undefined) call.status = early ? "failed" : "succeeded";
          calls.set(block.id, call);
          if (["bash", "edit", "write"].includes(block.name))
            remember(landmarks.actionCalls, callEntry);
          if (block.name === "ask_user_question")
            remember(landmarks.confirmations, callEntry);
        }
        return;
      }
      if (
  ...
  return {
    ...
    branchIssue: index.branchIssue,
    changedDuringRead:
      index.changedDuringRead ||
      after.size !== index.size ||
      after.mtimeMs !== index.mtimeMs,
  };
}
...
function safeValue(value: unknown, maxChars: number, depth = 0): unknown {
  ...
  if (typeof value === "string")
    return truncateLine(redactText(value), maxChars).text;
  if (value === null || typeof value === "number" || typeof value === "boolean")
    return value;
  if (Array.isArray(value))
    return value
      .slice(0, 10)
      .map((item) => safeValue(item, maxChars, depth + 1));
    ...
    Object.entries(value)
      ...
      .map(([key, item]) => [
        ...
        SENSITIVE_KEY.test(key)
          ? "[redacted]"
          : safeValue(item, maxChars, depth + 1),
      ]),
  );
}
...
function safeJson(value: unknown, maxChars: number): string {
  return truncateLine(
    JSON.stringify(safeValue(value, Math.min(maxChars, 500))),
    maxChars,
  ).text;
}
  ...
  return content
    .map((block) => {
      ...
      if (block.type === "text" && typeof block.text === "string")
        return redactText(block.text);
      if (block.type === "image")
        return `[image ${stringValue(block.mimeType) ?? "unknown"}]`;
  ...
  if (entry.type === "message" && isObject(entry.message)) {
    ...
    if (message.role === "assistant") {
      ...
      if (Array.isArray(message.content)) {
        const toolCalls = message.content
          .filter(
            (block): block is JsonObject =>
              isObject(block) && block.type === "toolCall",
          )
          .slice(0, 10)
          .map((block) => ({
            id: block.id,
            name: block.name,
            arguments: safeJson(block.arguments, Math.min(maxChars, 500)),
          }));
        if (toolCalls.length) normalized.toolCalls = toolCalls;
      }
      if (isObject(message.usage))
        normalized.usage = safeValue(message.usage, maxChars);
    ...
    if (entry.customType === SYSTEM_PROMPT_TYPE && isObject(entry.data)) {
      ...
      if (typeof entry.data.prompt === "string")
        normalized.prompt = clip(redactText(entry.data.prompt));
    }
...
function outlineEntry(
  ...
  if (entry.type === "message" && isObject(entry.message)) {
    ...
    if (entry.message.role === "toolResult")
      outlined.isError = entry.message.isError === true;
  } else if (entry.type === "custom") {
...
function expandedContextIds(
  ...
  for (const [name, value] of [
    ...
    if (!Number.isInteger(value) || value < 0 || value > 5)
      throw new Error(`context.${name} must be an integer between 0 and 5`);
  }
  const ordered = [...index.entries.values()].filter(
    (entry) => !selected || selected.has(entry.id),
  );
  ...
  for (const seed of [...ids]) {
    ...
    for (
      let i = Math.max(0, position - before);
      i <= Math.min(ordered.length - 1, position + after);
      i++
    )
  ...
  if (context.toolPair) {
    const callIds = new Set(
      [...ids].flatMap((id) => index.entries.get(id)?.toolCallIds ?? []),
    );
    for (const entry of ordered)
      if (entry.toolCallIds.some((id) => callIds.has(id))) ids.add(entry.id);
  }
  return ids;
}
...
export async function extractSession(
  ...
  if (!Number.isInteger(maxChars) || maxChars < 100 || maxChars > 2000)
    throw new Error("maxChars must be an integer between 100 and 2000");
  if (!Number.isInteger(totalChars) || totalChars < 1000 || totalChars > 30000)
    throw new Error("totalChars must be an integer between 1000 and 30000");
  if (!Number.isInteger(cursor) || cursor < 2)
    throw new Error("cursor must be an integer greater than or equal to 2");
  if (options.agentName && !AGENT_NAME_PATTERN.test(options.agentName))
    throw new Error(`Invalid agent name: ${options.agentName}`);
  const invalidField = options.fields?.find(
    (field) => !EXTRACT_FIELD_SET.has(field),
  );
  if (invalidField)
    throw new Error(`Unsupported projection field: ${invalidField}`);
  if (options.context && !options.entryIds?.length)
    throw new Error("entryIds are required when context is requested");
  if (
    options.context &&
    (options.entryTypes?.length ||
      options.roles?.length ||
      options.toolNames?.length ||
      options.toolCallIds?.length ||
      options.query ||
      options.fromTimestamp ||
      options.toTimestamp)
  ) {
    throw new Error(
      "context cannot be combined with content filters; entryIds define the evidence seeds",
    );
  }
  const from =
    options.fromTimestamp === undefined
      ? undefined
      : Date.parse(options.fromTimestamp);
  const to =
    options.toTimestamp === undefined
      ? undefined
      : Date.parse(options.toTimestamp);
  if (from !== undefined && !Number.isFinite(from))
    throw new Error("fromTimestamp must be an ISO timestamp");
  if (to !== undefined && !Number.isFinite(to))
    throw new Error("toTimestamp must be an ISO timestamp");

  const index = await indexSession(inputPath, options.signal);
  const selected = selection(index, scope, options.agentName);
  const contextIds = options.context
    ? expandedContextIds(index, selected, options.entryIds!, options.context)
    : undefined;
  ...
  const includes = (list: string[] | undefined, value: string | undefined) =>
    !list?.length || (!!value && list.includes(value));
  ...
  await forEachSelectedEntry(
    ...
    (entry, line, rawLine) => {
      ...
      if (contextIds) {
        ...
        if (
          options.toolNames?.length &&
          !toolNames.some((name) => options.toolNames!.includes(name))
        )
          return;
        if (
          options.entryIds?.length &&
          !options.entryIds.includes(String(entry.id))
        )
          return;
        const toolCallIds = entryToolCallIds(entry);
        if (
          options.toolCallIds?.length &&
          !toolCallIds.some((id) => options.toolCallIds!.includes(id))
        )
          return;
        if (
          from !== undefined &&
          (!Number.isFinite(timestampMs) || timestampMs < from)
        )
          return;
        if (
          to !== undefined &&
          (!Number.isFinite(timestampMs) || timestampMs > to)
        )
          return;
      }
      const base =
        view === "outline"
          ? outlineEntry(entry, line, rawLine.length)
          : normalizeEntry(entry, line, maxChars);
      ...
      if (
        ...
        return;
      let eventChars =
        JSON.stringify(normalized).length + (events.length ? 1 : 0);
      ...
      if (
        events.length === limit ||
        (events.length > 0 && returnedChars + eventChars > totalChars)
      ) {
        nextCursor = line;
  ...
  return {
    ...
    returnedChars,
    coveredLines:
      firstLine === null ? null : { first: firstLine, last: lastLine },
    nextCursor,
...
function statsText(stats: SessionStats): string {
  const tools =
    Object.entries(stats.toolCalls.byTool)
      .map(
        ([name, item]) =>
          `- ${name}: ${item.total} total, ${item.succeeded} succeeded, ${item.failed} failed, ${item.unresolved} unresolved`,
      )
      .join("\n") || "- (none)";
  const landmarkLines = (
    Object.entries(stats.landmarks) as Array<
      [keyof SessionLandmarks, SessionLandmark[]]
    >
  )
  ...
  if (stats.landmarks.largestEntries.length) {
    landmarkLines.push(
      `- largestEntries: ${stats.landmarks.largestEntries.map((item) => `${item.id}(${item.chars})`).join(", ")}`,
    );
  }
  return [
    `Session: ${stats.path}`,
    ...
    ...(stats.invalidLineCount
      ? [
          `Invalid JSONL lines: ${stats.invalidLineCount} (sample: ${stats.invalidLines.join(", ")})`,
        ]
      : []),
    ...(stats.duplicateIdCount
      ? [
          `Duplicate entry IDs: ${stats.duplicateIdCount} (sample: ${stats.duplicateIds.join(", ")})`,
        ]
      : []),
    ...(stats.branchIssue ? [`Branch issue: ${stats.branchIssue}`] : []),
    ...(stats.changedDuringRead
      ? [
          "Warning: the session changed while it was read; metrics are a partial snapshot.",
        ]
      : []),
  ].join("\n");
}
...
export function registerSessionTools(pi: ExtensionAPI): void {
  pi.on("session_start", () => {
    try {
      pi.setActiveTools(
        pi.getActiveTools().filter((name) => !SESSION_TOOL_NAMES.has(name)),
      );
    } catch {
  ...
  pi.registerTool({
    ...
    description:
      "Finds the most recent persisted pi JSONL sessions whose active branch contains a named pi-agent activation. Use before auditing when no session path was supplied.",
    promptSnippet: "Find the latest persisted session for a named pi-agent",
    promptGuidelines: [
      "Prefer the first match and report skipped or malformed sessions as audit limitations.",
    ],
    parameters: Type.Object({
      agentName: Type.String({
        pattern: AGENT_NAME_PATTERN.source,
        description: "Exact pi-agent name",
      }),
      limit: Type.Optional(
        Type.Integer({
          minimum: 1,
          maximum: 20,
          description: "Maximum matches, newest first (default 1)",
        }),
      ),
      sessionRoot: Type.Optional(
        Type.String({
          description: "Session directory; defaults to ~/.pi/agent/sessions",
        }),
      ),
      excludePath: Type.Optional(
        Type.String({
          description:
            "Exact session .jsonl path to exclude, typically the current audit session",
        }),
      ),
    }),
    async execute(_toolCallId, params, signal) {
      const result = await findAgentSessions(params.agentName, {
      ...
      const text = result.matches.length
        ? result.matches
            .map(
              (item, i) =>
                `${i + 1}. ${item.path}\n   last=${item.lastTimestamp ?? "unknown"} activeAtLeaf=${item.activeAtLeaf} segment=${item.latestSegment.startEntryId}..${item.latestSegment.endEntryId}`,
            )
            .join("\n")
  ...
  pi.registerTool({
    ...
    description:
      "Computes branch-aware metrics and compact evidence landmarks (requests, finals, failures, action calls, confirmations, compactions, and largest entries) from a pi session JSONL. Use before extraction.",
    promptSnippet:
      "Compute metrics and evidence landmarks for a pi JSONL session or one agent segment",
    promptGuidelines: [
      "Treat unresolved tool calls and changedDuringRead as evidence that the snapshot may be incomplete.",
    ],
    parameters: Type.Object({
      path: Type.String({ description: "Path to a pi session .jsonl file" }),
      scope: Type.Optional(
        Type.Union([
          Type.Literal("agent-segment"),
          Type.Literal("active-branch"),
          Type.Literal("whole-tree"),
        ]),
      ),
      agentName: Type.Optional(
        Type.String({
          pattern: AGENT_NAME_PATTERN.source,
          description: "Required for agent-segment scope",
        }),
      ),
    }),
    async execute(_toolCallId, params, signal) {
      const result = await calculateSessionStats(params.path, {
        scope: params.scope,
        agentName: params.agentName,
        signal,
      });
      return {
        content: [{ type: "text", text: statsText(result) }],
        details: result,
      };
    },
  });
  ...
  pi.registerTool({
    ...
    description:
      "Returns a globally bounded, branch-aware outline or detailed evidence from a pi session. Supports projections and causal context around known entry IDs; thinking and obvious credentials are omitted.",
    promptSnippet:
      "Outline a session, then extract causal evidence by landmark entry ID without loading the whole JSONL",
    promptGuidelines: [
      "Start with session_stats landmarks; use outline only when structure is unclear, then request evidence by entryIds with context.",
    ],
    parameters: Type.Object({
      path: Type.String({ description: "Path to a pi session .jsonl file" }),
      scope: Type.Optional(
      ...
      view: Type.Optional(
        Type.Union([Type.Literal("evidence"), Type.Literal("outline")], {
          description:
            "outline omits content and arguments; evidence is detailed (default)",
        }),
      ),
      fields: Type.Optional(
        Type.Array(
          Type.Union(EXTRACT_FIELDS.map((field) => Type.Literal(field))),
          {
            maxItems: 20,
            description: "Projection; line and id are always retained",
          },
        ),
      ),
      ...
      query: Type.Optional(
        Type.String({
          maxLength: 200,
          description: "Case-insensitive literal search on normalized output",
        }),
      ),
      ...
      cursor: Type.Optional(
        Type.Integer({
          minimum: 2,
          description: "JSONL line cursor returned by the previous call",
        }),
      ),
      limit: Type.Optional(
        Type.Integer({
          minimum: 1,
          maximum: 50,
          description: `Maximum events (default ${DEFAULT_LIMIT})`,
        }),
      ),
      maxChars: Type.Optional(
        Type.Integer({
          minimum: 100,
          maximum: 2000,
          description: `Maximum text characters per event (default ${DEFAULT_MAX_CHARS})`,
        }),
      ),
      totalChars: Type.Optional(
        Type.Integer({
          minimum: 1000,
          maximum: 30000,
          description: "Global serialized event budget (default 12000)",
        }),
      ),
        ...
        Type.Object(
          {
            ...
            toolPair: Type.Optional(
              Type.Boolean({
                description: "Include matching tool call/result entries",
              }),
            ),
          },
          {
            description:
              "Causal expansion around entryIds; cannot be combined with content filters",
          },
        ),
      ),
    }),
    async execute(_toolCallId, params, signal) {
      ...
      return {
        content: [
          {
            type: "text",
            text: `${rendered.content}${rendered.truncated ? "\n[output truncated]" : ""}\n${coverage}`,
          },
        ],
