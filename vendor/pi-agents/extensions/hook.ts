 ...
 *       thinking level). Opt-in via PI_DEFAULT_AGENT or the persisted global /
 *       workspace default.
 *
...
import { findAgent, resolveTools } from "./registry.js";
...
function findToolsArray(payload: unknown): unknown[] | null {
  ...
  const nested = (k: string) =>
    (p[k] as Record<string, unknown> | undefined)?.tools;
  for (const c of [
    p.tools,
    nested("body"),
    nested("request"),
...
export function registerHooks(
  ...
  options?: {
    /** Fixed name used by tests or embedders; otherwise config is resolved per cwd. */
  ...
  async function activateDefaultAgent(ctx: ExtensionContext): Promise<void> {
    const agentName =
      options?.autoActivateAgentName ?? getDefaultAgentName(ctx.cwd);
    ...
    try {
      ...
      try {
        ctx.ui.setStatus(
          "agent",
          ctx.ui.theme.fg("accent", `Agent: ${agent.name}`),
        );
      } catch {}
      try {
        ctx.ui.notify(`Agent "${agent.name}" activated automatically`, "info");
      } catch {}
    } catch {
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    if (activeAgentName) {
      ...
      const { prompt, missingSkills, missingDelegates } = composeAgentPrompt(
        pi,
        ctx.cwd,
        agent,
        {
          systemPromptOptions: (event as { systemPromptOptions?: unknown })
            .systemPromptOptions,
      ...
      if (missingSkills.length) {
        try {
          ctx.ui.notify(
            `Agent "${agent.name}": unknown skills ignored: ${missingSkills.join(", ")}`,
            "warning",
          );
        } catch {
          /* no UI */
      ...
      if (missingDelegates.length) {
        try {
          ctx.ui.notify(
            `Agent "${agent.name}": unknown delegates ignored: ${missingDelegates.join(", ")}`,
            "warning",
          );
        } catch {
          /* no UI */
      ...
      try {
        const toolsToSet = resolveTools(agent);
  ...
  pi.on("before_provider_request", (event, ctx) => {
    ...
    try {
      ...
      if (agentName) {
        const agent = findAgent(ctx.cwd, agentName);
        if (agent) allowSet = new Set(resolveTools(agent));
      }
    } catch {
    ...
    try {
      ...
      if (allowSet && !tools && !warnedGuardBypass) {
        ...
        try {
          ...
          /* no UI available in this context */
        }
      }
