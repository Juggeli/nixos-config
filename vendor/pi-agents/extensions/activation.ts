...
export async function applyAgent(
  ...
  // Set model before thinking level: set_model re-applies the global thinking
  // default, so the agent's thinkingLevel must come last to stick.
  if (agent.model) {
 ...
 */
export function persistActiveAgent(
  pi: ExtensionAPI,
  name: string | null,
): void {
  try {
