...
const { registerArchitectTools } = await import(
  pathToFileURL(join(compiled, "architect-tools.js")).href
);
...
function harness(cwd: string) {
  ...
  const execute = (name: string, params: object) =>
    tools
      .get(name)
      .execute("call", params, new AbortController().signal, () => {}, ctx);
...
test("validates a project agent written to its final path", async () => {
  ...
  try {
    ...
    const validation = await execute("agent_validate", {
      scope: "project",
      name: "reviewer",
    });
...
test("validates a bundled agent with its uppercase filename", async () => {
  ...
  const bundledMarkdown = markdown.replace(
    "name: reviewer",
...
test("validates delegate references against discovered agents", async () => {
  const cwd = mkdtempSync(join(tmpdir(), "pi-agents-architect-"));
  // Create the agent under test
  const agentsDir = join(cwd, ".pi", "agents");
  mkdirSync(agentsDir, { recursive: true });
  // Also create a valid target agent so the reference resolves
  writeFileSync(
    join(agentsDir, "explorer.md"),
    `---\nname: explorer\ndescription: Explores code\n---\n\nExplore.\n`,
  );
  const orchestratorMd = `---\nname: orchestrator\ndescription: Routes tasks\ndelegate: [explorer]\ntools: [read]\n---\n\nAct.\n`;
  writeFileSync(join(agentsDir, "orchestrator.md"), orchestratorMd);
  ...
  try {
    ...
    assert.equal(validation.details.valid, true);
    assert.deepEqual(validation.details.normalized?.delegate, ["explorer"]);
  } finally {
    rmSync(cwd, { recursive: true, force: true });
  }
});

test("rejects unknown delegate references", async () => {
  const cwd = mkdtempSync(join(tmpdir(), "pi-agents-architect-"));
  const agentsDir = join(cwd, ".pi", "agents");
  mkdirSync(agentsDir, { recursive: true });
  writeFileSync(
  ...
  try {
    ...
    assert.equal(validation.details.valid, false);
    assert.ok(
      validation.details.diagnostics.some(
        (d: { code: string }) => d.code === "unknown_delegate",
      ),
    );
  } finally {
    rmSync(cwd, { recursive: true, force: true });
...
test("rejects missing and symlinked final agents", async () => {
  ...
  try {
    const { execute } = harness(cwd);
    const missing = await execute("agent_validate", {
      scope: "project",
      name: "reviewer",
    });
    ...
    const symlinked = await execute("agent_validate", {
      scope: "project",
      name: "reviewer",
    });
