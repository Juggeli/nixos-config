...
const source = readFileSync(
  join(process.cwd(), "extensions", "skill-tools.ts"),
  "utf8",
);
...
const { registerSkillTools } = await import(
  pathToFileURL(join(compiled, "skill-tools.js")).href
);
...
test("validates and atomically saves a multi-file skill while preserving untouched files", async () => {
  ...
  try {
    ...
    const validation = await execute("skill_validate", {
      scope: "project",
      name: "example-skill",
    });
    assert.equal(validation.details.valid, true);
    assert.deepEqual(
      validation.details.changes.map((item: { path: string }) => item.path),
      ["references/guide.md", "SKILL.md"],
    );

    const created = await execute("skill_save", {
      scope: "project",
      name: "example-skill",
    });
    assert.equal(created.details.verified, true);
    assert.equal(
      readFileSync(join(target, "references", "guide.md"), "utf8"),
      "# Guide\n",
    );
    ...
    writeFileSync(
      join(draft, "SKILL.md"),
      skill("example-skill", "Follow the improved workflow."),
    );
    const updated = await execute("skill_save", {
      scope: "project",
      name: "example-skill",
    });
...
test("rejects mismatched names and symlinks, and hides skill tools by default", async () => {
  ...
  try {
    ...
    const invalid = await instance.execute("skill_save", {
      scope: "project",
      name: "example-skill",
    });
